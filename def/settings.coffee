module.exports =
	...
	init: () ->
		...
		pref.find('label[for="editor.useShadowDOM"]').next('.setting-description').html('如果您遇到擴充套件或者佈景主題在變更軟體外觀上的問題，請取消打勾此項設定，可禁止 Atom 使用 Shadow DOM 產生元件。您也可以針對該擴充套件或佈景主題到 GitHub 提出 issue。(本設定之後將被廢棄。)')
